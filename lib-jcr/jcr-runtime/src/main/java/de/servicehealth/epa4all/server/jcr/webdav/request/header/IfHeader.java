...
public class IfHeader implements Header {
     ...
     * @return
     */
    private IfHeader.IfHeaderMap parseTagged(StringReader reader) {
        IfHeader.IfHeaderMap map = new IfHeader.IfHeaderMap();
        try {
            while (true) {
     ...
     *
     * @return An <code>ArrayList</code> of {@link IfHeader.IfList} entries.
     */
     ...
     * @param reader The <code>StringReader</code> to read from for parsing
     *
     * @return The {@link IfHeader.IfList} for the input <em>IfList</em>.
     *
     * @throws IOException if a problem occurs during reading.
     */
    private IfHeader.IfList parseIfList(StringReader reader) throws IOException {
        IfHeader.IfList res = new IfHeader.IfList();
        ...
        while (true) {
            ...
            switch (nextChar) {
                ...
                case '<':
                    ...
                    if (word != null) {
                        res.add(new IfHeader.IfListEntryToken(word, positive));
                ...
                case '[':
                    ...
                    word = readWord(reader, ']');
                    if (word != null) {
                        res.add(new IfHeader.IfListEntryEtag(word, positive));
                        positive = true;
                    }
                    break;
    ...
    /**
     * The <code>IfListEntryToken</code> extends the {@link IfHeader.IfListEntry}
     * abstract class to represent an entry for token matching.
     */
    private static class IfListEntryToken extends IfHeader.IfListEntry {
     ...
     */
    private static class IfListEntryEtag extends IfHeader.IfListEntry {
     ...
     * with the limitation to only support adding {@link IfHeader.IfListEntry} objects
     ...
     * <p>
     */
    private static class IfList extends ArrayList<IfHeader.IfListEntry> {

        @Serial
        private static final long serialVersionUID = 1637983579617683046L;

        /**
         * Adds the {@link IfHeader.IfListEntry} at the end of the list.
         *
         * @param entry The {@link IfHeader.IfListEntry} to add to the list
         *
         * @return <code>true</code> (as per the general contract of Collection.add).
         */
        @Override
        public boolean add(IfHeader.IfListEntry entry) {
            return super.add(entry);
        }

        /**
         * Adds the {@link IfHeader.IfListEntry} at the indicated position of the list.
         *
         * @param index
         * @param entry
         *
         ...
         */
        @Override
        public void add(int index, IfHeader.IfListEntry entry) {
            super.add(index, entry);
        }

        /**
         * Returns <code>true</code> if all {@link IfHeader.IfListEntry} objects in the
        ...
        public boolean match(String token, String etag) {
            ...
            for (int i=0; i < size(); i++) {
                IfHeader.IfListEntry ile = get(i);
    ...
    /**
     * The <code>IfHeaderList</code> class implements the {@link IfHeader.IfHeaderInterface}
     * interface to support untagged lists of {@link IfHeader.IfList}s. This class
     * implements the data container for the production :
     * <pre>
     Untagged = { "(" IfList ")" } .
     * </pre>
     */
    private static class IfHeaderList extends ArrayList<IfHeader.IfList> implements IfHeader.IfHeaderInterface {

        @Serial
        private static final long serialVersionUID = 1908088258055057995L;

        /**
         * Matches a list of {@link IfHeader.IfList}s against the token and etag. If any of
         * the {@link IfHeader.IfList}s matches, the method returns <code>true</code>.
         * On the other hand <code>false</code> is only returned if non of the
         * {@link IfHeader.IfList}s match.
         *
         ...
         * @param etag The ETag value to compare.
         *
         * @return <code>True</code> if any of the {@link IfHeader.IfList}s matches the token
         *      and etag, else <code>false</code> is returned.
         */
        public boolean matches(String resource, String token, String etag) {
            log.debug("matches: Trying to match token="+token+", etag="+etag);

            for (IfHeader.IfList il : this) {
    ...
    /**
     * The <code>IfHeaderMap</code> class implements the {@link IfHeader.IfHeaderInterface}
     * interface to support tagged lists of {@link IfHeader.IfList}s. This class
     * implements the data container for the production :
     * <pre>
     Tagged = { "&lt;" Word "&gt;" "(" IfList ")" } .
     * </pre>
     */
    private static class IfHeaderMap extends HashMap<String, IfHeader.IfHeaderList> implements IfHeader.IfHeaderInterface {

        @Serial
        private static final long serialVersionUID = -3600909350184201893L;
        ...
        public boolean matches(String resource, String token, String etag) {
            ...
            IfHeader.IfHeaderList list = get(resource);
            if (list == null) {
