...
import de.servicehealth.epa4all.server.jcr.webdav.request.header.CodedUrlHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.DepthHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.IfHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.LabelHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.OverwriteHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.PollTimeoutHeader;
import de.servicehealth.epa4all.server.jcr.webdav.request.header.TimeoutHeader;
...
import static de.servicehealth.epa4all.server.jcr.WebdavServlet.getContentCodings;
...
public class JWebdavRequestImpl implements JWebdavRequest, DavConstants, ContentCodingAwareRequest {

    private static final Logger log = LoggerFactory.getLogger(JWebdavRequestImpl.class);
     ...
     * @param httpRequest
     * @param factory
     * @param createAbsoluteURI defines if we must create an absolute URI. if false a absolute path will be created
     */
    public JWebdavRequestImpl(HttpServletRequest httpRequest, DavLocatorFactory factory, boolean createAbsoluteURI) {
            ...
            try {
                ...
                if (auth == null) {
                    ...
                    if (ref.startsWith("//") || !ref.startsWith("/")) {
                        ...
                        throw new DavException(SC_BAD_REQUEST);
                    }
                    ...
                    throw new DavException(SC_FORBIDDEN);
                }
        ...
        for (int i = 1; i <= len; i++) {
            ...
            if ("gzip".equals(s)) {
                ...
                try {
                    ...
                    throw new IOException(davException);
                } catch (Exception ex) {
    ...
    public List<String> getRequestContentCodings() {
        if (requestContentCodings == null) {
            requestContentCodings = getContentCodings(httpRequest);
        }
    ...
    private void parsePropFindRequest() throws DavException {
        ...
        if (!XML_PROPFIND.equals(root.getLocalName())) {
            ...
            throw new DavException(SC_BAD_REQUEST, "PropFind-Request has no <propfind> tag.");
        }
        ...
        if (propfindTypeFound > 1) {
            ...
            throw new DavException(SC_BAD_REQUEST, "Multiple top-level propfind instructions");
        }
        ...
        if (include != null) {
            if (propfindType == PROPFIND_ALL_PROP) {
                ...
                throw new DavException(SC_BAD_REQUEST, "<include> goes only with <allprop>");
    ...
    private void parsePropPatchRequest() throws DavException {
        DavPropertySet proppatchSet = new DavPropertySet();
        proppatchList = new ArrayList<>();

        Document requestDocument = getRequestDocument();

        if (requestDocument == null) {
            throw new DavException(SC_BAD_REQUEST, "Invalid request body.");
        }

        Element root = requestDocument.getDocumentElement();
        if (!DomUtil.matches(root, XML_PROPERTYUPDATE, NAMESPACE)) {
            log.warn("PropPatch-Request has no <DAV:propertyupdate> tag.");
            throw new DavException(SC_BAD_REQUEST, "PropPatch-Request has no <propertyupdate> tag.");
        }
    ...
    private static boolean isStrongETag(String eTag) {
        return eTag != null && !eTag.isEmpty() && !eTag.startsWith("W\\");
    }
    ...
    public ServletInputStream getInputStream() throws IOException {
        return new JWebdavRequestImpl.MyServletInputStream(getDecodedInputStream());
    }
