...
public class VauMessage3Transformer implements ResponseDefinitionTransformerV2 {
    ...
    public ResponseDefinition transform(ServeEvent serveEvent) {
        ...
        try {
            ...
            return switch (path) {
                case "/epa/authz/v1/getNonce" -> prepareVauResponse(getFixture("GetNonce200Response.json"));
                ...
                case "/epa/authz/v1/send_authcode_sc" -> prepareVauResponse(getFixture("SendAuthCodeSC200Response.json"));
                default -> throw new IllegalArgumentException("Unknown path: " + path);
            };
        }
    }

    private ResponseDefinition prepareVauResponse(String payload) {
        return prepareVauResponse(payload, null);
    }

    private ResponseDefinition prepareVauResponse(String payload, String location) {
        ...
        byte[] innerResponse = httpParcel.toString(false).getBytes();

        return new ResponseDefinitionBuilder()
            .withHeader(CONTENT_TYPE, "application/cbor")
            .withHeader(CONTENT_LENGTH, String.valueOf(innerResponse.length))
            .withStatus(200)
            .withBody(innerResponse).build();
    }
